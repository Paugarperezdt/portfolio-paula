    ...
    <div class="grid gap-8 sm:grid-cols-2 lg:grid-cols-2">
        @foreach ($proyectos as $proyecto)
            <article class="border border-gray-200   hover:shadow-md transition p-5 flex flex-col justify-between min-h-[400px]">
                ...
                <h3 class="text-lg font-semibold text-black mb-2">{{ $proyecto['titulo'] }}</h3>

                <p class="text-sm text-gray-700 flex-grow">{{ $proyecto['descripcion'] }}</p>
                ...
                <a href="{{ route('proyectos.detalle', ['slug' => $proyecto['slug']]) }}"
        ...
        @foreach ($tecnologias as $tec)
            <div class="group flex flex-col items-center justify-center transform transition duration-300 hover:scale-105">
                <div class="w-20 h-20 aspect-[1/1] flex items-center justify-center">
                    <img src="{{ asset('images/icons/' . $tec['archivo']) }}"
                         alt="Logo de {{ $tec['nombre'] }}"
                         class="max-h-full max-w-full object-contain grayscale group-hover:grayscale-0 transition duration-300 ease-in-out {{ $tec['clase_extra'] ?? '' }}" />
                </div>
                <p class="mt-3 text-sm font-medium text-gray-800">{{ $tec['nombre'] }}</p>
            </div>
        @endforeach
    </div>
</section>
